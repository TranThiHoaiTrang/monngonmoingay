...
trait Wp {
	 ...
	 * @return string|void
	 */
	public static function siteTitleOrLogo( bool $echo = true, string $home_heading = 'div', string $class = 'logo' ) {
		$is_home_or_front_page = is_home() || is_front_page();
		$tag                   = $is_home_or_front_page ? $home_heading : 'div';

		if ( function_exists( 'the_custom_logo' ) && has_custom_logo() ) {
			$logo = get_custom_logo();
			$html = $is_home_or_front_page ? '<' . $tag . ' id="logo" class="' . $class . '">' . $logo . '</' . $tag . '>' : $logo;
		} else {
			$html = '<' . esc_attr( $tag ) . ' class="' . $class . '"><a title href="' . self::home() . '" rel="home">' . esc_html( get_bloginfo( 'name' ) ) . '</a></' . esc_attr( $tag ) . '>';
	...
	public static function primaryTerm( $post, string $taxonomy = '' ): mixed {
		...
		if ( ! $taxonomy ) {
			...
			$ehd_post_type_terms_arr = apply_filters( 'ehd_post_type_terms', [] );
			if ( ! empty( $ehd_post_type_terms_arr ) ) {
				foreach ( $ehd_post_type_terms_arr as $_post_type => $_taxonomy ) {
