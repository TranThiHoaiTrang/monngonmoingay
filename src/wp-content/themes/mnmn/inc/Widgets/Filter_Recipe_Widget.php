...
class Filter_Recipe_Widget extends Abstract_Widget {
	public function __construct() {
		$this->widget_description = __( 'Filter recipes list.', EHD_TEXT_DOMAIN );
		$this->widget_name        = __( 'MNMN - Filter Recipe *', EHD_TEXT_DOMAIN );
		$this->settings           = [
			'title' => [
				'type'  => 'text',
				'std'   => '',
	...
	public function widget( $args, $instance ) {
		...
		<div class="container flex flex-col px-4 mt-10 lg:mt-14<?=$css_class?>">
            ...
            <?php if ( $recipe_cat ) : ?>
			<div class="flex mt-5 gap-12 justify-between">
				<ul class="tabs">
                    <?php
                    foreach ( $recipe_cat as $i => $r_cat_id ) :
	                    $r_cat = get_term_by( 'id', $r_cat_id, 'danhmuc_monan' );
                        $tab_active = '';
                        if ( 0 == $i ) $tab_active = ' tab-active';
                    ?>
					<li class="tab<?=$tab_active?>">
						<a href="<?php echo get_term_link( $r_cat, 'danhmuc_monan' ); ?>" title="<?php echo esc_attr( $r_cat->name ); ?>"><?php echo $r_cat->name; ?></a>
					</li>
                    <?php endforeach; ?>
				</ul>
			</div>
            ...
            <?php
            $target_term_id = $recipe_cat[0] ?? 0;

            $slides_query = false;
            if ( $target_term_id ) {
                $term = get_term_by( 'id', $target_term_id, 'danhmuc_monan' );
                if ( $term ) {
                    $slides_query = Helper::queryByTerm( $term, 'monan', true, $number, [ 'menu_order' => 'ASC' ] );
                }
            }
			...
			<div class="relative has-[.slider]:-mx-4 group">
                ...
                <div class="flex lg:gap-8 justify-between slider slider-3 flex-wrap flex-col lg:flex-row">
                    ...
                    // Load slides loop.
                    $i = 0;
                    while ( $slides_query->have_posts() && $i < $number ) : $slides_query->the_post();
	                    $post = get_post();
	                    $i++;
                    ...
                    <div class="hidden lg:!flex flex-col mt-6 lg:mt-5 gap-5 w-full group-has-[.group-1]:lg:flex-row group-has-[.slider]:px-4 group-has-[.group-2]:flex-[0_1_calc(50%_-_20px)] group-has-[.group-3]:flex-[0_1_calc(33%_-_20px)] group-has-[.group-4]:flex-[0_1_calc(24%_-_20px)] group-has-[.group-1]:lg:border group-has-[.group-1]:lg:!p-5 group-has-[.group-1]:lg:rounded-2xl [&:nth-child(1)]:!flex [&:nth-child(2)]:!flex  pb-5 lg:pb-10">
						<div class="relative rounded-2xl overflow-hidden group-has-[.group-1]:flex-[0_1_calc(50%_-_20px)]<?=$ratio_class?>">
                            ...
                            <a href="#" data-tooltip-target="spec-<?=$i?>" data-tooltip-placement="top" class="absolute top-4 right-4 w-12 h-12 rounded-full border border-[var(--Gray-02)] bg-[var(--Gray-03)] flex items-center justify-center text-[var(--Gray-02)] hover:text-[var(--Primary-02)] hover:border-[var(--Primary-02)] hover:bg-[var(--Primary-03)] transition-all">
                                <span class="icon-[ph--heart-fill] text-2xl"></span>
                            </a>
						</div>
						<div id="spec-<?=$i?>" role="tooltip"
