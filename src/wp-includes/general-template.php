...
function paginate_links_post_ehd( $args = '' ) {
	global $wp_query, $wp_rewrite;
	...
	if ( $args['prev_next'] && $current && 1 < $current ) :
		...
		$page_links[] = sprintf(
			'<li class="page"><a class="prev page-numbers" href="%s">%s</a></li>',
			/**
			 * Filters the paginated links for the given archive pages.
			 *
			 * @since 3.0.0
	...
	for ( $n = 1; $n <= $total; $n++ ) :
		if ( $n == $current ) :
			$page_links[] = sprintf(
				'<li class="page page-active current"><span aria-current="%s" class="page-numbers current">%s</span></li>',
				esc_attr( $args['aria_current'] ),
				$args['before_page_number'] . number_format_i18n( $n ) . $args['after_page_number']
			);
			...
			if ( $args['show_all'] || ( $n <= $end_size || ( $current && $n >= $current - $mid_size && $n <= $current + $mid_size ) || $n > $total - $end_size ) ) :
				...
				$page_links[] = sprintf(
					'<li class="page"><a class="page-numbers" href="%s">%s</a></li>',
					/** This filter is documented in wp-includes/general-template.php */
					esc_url( apply_filters( 'paginate_links', $link ) ),
					$args['before_page_number'] . number_format_i18n( $n ) . $args['after_page_number']
				);
	...
	if ( $args['prev_next'] && $current && $current < $total ) :
		...
		$page_links[] = sprintf(
			'<li class="page"><a class="next page-numbers" href="%s">%s</a></li>',
			/** This filter is documented in wp-includes/general-template.php */
			esc_url( apply_filters( 'paginate_links', $link ) ),
			$args['next_text']
		);
